          ...
          {navItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon className={`w-4.5 h-4.5 flex-shrink-0 ${active ? 'text-white' : 'text-slate-500 group-hover:text-slate-300'}`} size={18} />
        ...
        <div className="px-4 py-4 border-t border-slate-700/60">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-slate-800">
            ...
            <div className="min-w-0">
              <p className="text-white text-xs font-semibold truncate">your-username</p>
              <p className="text-slate-500 text-xs truncate">GitHub 接続済み</p>
            </div>
          </div>
        </div>
      </aside>
